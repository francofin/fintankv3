...
def stock_search(request):
    ...
    if request.method == 'POST':
        ...
        ticker_request = ticker_request_raw.upper()
        if '.' in ticker_request:
            index = ticker_request.find('.')
            ticker_request = ticker_request[0:index]
        else:
            ticker_request = ticker_request
        ticker_details = json.loads(requests.get(f'https://fmpcloud.io/api/v3/quote/'+ticker_request+'?apikey='+api).content)
        ticker_name = ticker_details[0]['name']
        ticker_exchange = ticker_details[0]['exchange']
        today = date.today()
        ...
        # Sector ranks
        sector_data = pd.DataFrame(json.loads(requests.get(f'https://fmpcloud.io/api/v3/stock-screener?sector='+sector+'&limit=300&apikey=3da6aaea4ffa4232c7ada6b09e15af62').content))
        ...
        analyst_recommendations_over_time = analyst_recommendations(ticker_request)
        colour2 = [ "#DA1717", "#7217DA", "#1799DA", "#17DA72", "#EFEF0B"]
        rec_output = list(analyst_recommendations_over_time.columns)
        analyst_recommendations_over_time['dates'] = pd.to_datetime(analyst_recommendations_over_time.index)

        rec_chart = figure(x_axis_type='datetime', width=1000, height=500, title = "Analyst Recommendations", toolbar_location=None, tools="hover", tooltips="@y")
        for (col, rand_col, leg) in zip(analyst_recommendations_over_time, colour2, rec_output):
            plot = rec_chart.line(analyst_recommendations_over_time['dates'], analyst_recommendations_over_time[col], color = rand_col, legend_label=leg, line_width=2)
        rec_chart.xaxis.axis_label = "Date"
        rec_chart.yaxis.axis_label = "Change"
        rec_chart.legend.location="top_left"
        rec_chart.legend.click_policy="hide"
        script8, div8 = components(rec_chart)
        cdn_js = CDN.js_files
        cdn_css = CDN.css_files
        ...
        return render(request, 'stock_search/stock_search.html', data)
